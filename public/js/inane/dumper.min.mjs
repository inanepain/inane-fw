 ...
 */
const out=console;class LogLevel{#options;constructor({name,value}){this.#options=arguments[0];Object.defineProperties(this,{name:{enumerable:!0,value:this.#options.name,},value:{enumerable:!0,value:this.#options.value,},})}
...
allows(level){if([defaults.level,level].includes(Dumper.OFF))return!1;if(level===undefined)return!0;return this.value-level.value<=0}}
const isStatic=(obj)=>obj===Dumper;const copyObject=(original)=>{return JSON.parse(JSON.stringify(original))}
const mergeOptions=(target,...source)=>{let key,i;for(i=0;i<source.length;i++)
for(key in source[i])
if(!(key in target)&&source[i].hasOwnProperty(key))target[key]=source[i][key];else try{if(target[key].constructor===Object&&source[i][key].constructor===Object)mergeOptions(target[key],source[i][key]);}catch(error){if(error.message.includes('target[key].constructor'))target[key]=source[i][key]}};const throttle=(func,limitDelay=1000,options={})=>{mergeOptions(options,{skipfirst:!1,context:func,});let inThrottle=options.skipfirst;let inDebounce;return function(){const args=arguments;const context=options.context;if(!inThrottle){inThrottle=!0;func.apply(context,args);inThrottle=setTimeout(()=>inThrottle=undefined,limitDelay)}else{clearTimeout(inDebounce);inDebounce=setTimeout(()=>func.apply(context,args),limitDelay)}}};const formatMessage=(messages,context,style=!1)=>{if(typeof context!==`undefined`&&Array.isArray(context.name)&&context.name.length>0){const name='['.concat(context.name.join(' - ')).concat(']');if(style)messages.unshift(Object.entries(style).join('; ').replaceAll(',',': ').concat(';'));messages.unshift((style?'%c':'').concat(name))}};const funcs=['log','trace','debug','info','warn','error','time','timeEnd','timeLog','timeStamp','count','countReset','getLevel','setLevel','children','assert'];const Children=new Map();const Counters={default:0};const LogLevels=(()=>{const LogLevels=Object.create(null);Object.defineProperties(LogLevels,{TRACE:{value:new LogLevel({value:1,name:'TRACE'}),enumerable:!0,},DEBUG:{value:new LogLevel({value:2,name:'DEBUG'}),enumerable:!0,},INFO:{value:new LogLevel({value:3,name:'INFO'}),enumerable:!0,},TIME:{value:new LogLevel({value:4,name:'TIME'}),enumerable:!0,},WARN:{value:new LogLevel({value:5,name:'WARN'}),enumerable:!0,},ERROR:{value:new LogLevel({value:8,name:'ERROR'}),enumerable:!0,},OFF:{value:new LogLevel({value:99,name:'OFF'}),enumerable:!0,},});Object.freeze(LogLevels);return LogLevels})();
/**
 * Dumper</br>
 ...
 * @see {@link https://unlicense.org/UNLICENSE UNLICENSE}
 */
class Dumper{static get VERSION(){return'2.5.0'}
get VERSION(){return this.constructor.VERSION}
#options;#children;static get kids(){return this.children()}
get kids(){return this.children()}
#context={name:[]};#counters={default:0};constructor({clear=!1,level=Dumper.WARN,bubbling={listen:!0,trigger:!0,},trickle=1000,assert={time:!1,hhmmss:!1,limit:0,},}=defaults){this.#options=this.#parseOptions((arguments[0]||defaults));if(!(this.#options.level instanceof LogLevel))this.#options.level=LogLevel.from(this.#options.level);for(const func of funcs)this[func]=Dumper[func].bind(this);this.group=out.group.bind(this);this.groupCollapsed=out.groupCollapsed.bind(this);this.groupEnd=out.groupEnd.bind(this);this.clear=out.clear.bind(this);this.trickle=throttle(this.debug,this.#options.trickle,{context:this});if(!this.dump)this.dump=()=>{};if(this.#options.clear)this.clear();}
#parseOptions(options){const picked={};Object.keys(defaults).forEach(key=>{if(options.hasOwnProperty(key))picked[key]=options[key]});mergeOptions(picked,defaults);picked.id=Date.now();if(options.linked===!0){this.get=Dumper.get.bind(this);this.#children=new Map();this.#context=options.context}else this.get=Dumper.get.bind(Dumper);return picked}
static get(name,{clear=!1,level=Dumper.WARN,bubbling={listen:!0,trigger:!0,},trickle=1000,assert={time:!1,hhmmss:!1,limit:0,},}=defaults){let options=arguments[1]??{};const children=(this===Dumper||this.#children===undefined)?Children:this.#children;if(!children.has(name)){if(this!==Dumper)mergeOptions(options,this.#options);if(options.level&&options.level.name)options.level=options.level.name;options=copyObject(options);options.level=LogLevel.from(options.level);let context=this===Dumper?{name:[]}:copyObject(this.#context);context.parent=this;context.name.push(name);options.linked=!0;options.context=context;const child=new Dumper(options);child.name=name;children.set(name,child)}
return children.get(name)}
static children(){const kids=Object.create(null);for(let[name,child]of(this!==Dumper&&this.#children||Children))kids[name]=child;return kids}
static get TRACE(){return LogLevels.TRACE}
static get DEBUG(){return LogLevels.DEBUG}
static get INFO(){return LogLevels.INFO}
static get TIME(){return LogLevels.TIME}
static get WARN(){return LogLevels.WARN}
static get ERROR(){return LogLevels.ERROR}
static get OFF(){return LogLevels.OFF}
...
static setLevel(level,bubbled=!1){let options=(this===Dumper||this.#options===undefined)?defaults:this.#options;if(!(level instanceof LogLevel))level=LogLevel.from(level);if(!bubbled||(this===Dumper||options.bubbling.listen))options.level=level;if((this===Dumper&&level===this.OFF)||(bubbled&&options.bubbling.trigger===!1))return;if(this!==Dumper&&this.#children===undefined)return this;if(options.bubbling.trigger)for(let child of(this!==Dumper&&this.#children||Children).values())child.setLevel(level,!0);return this}
...
static dump(...messages){if(out.dump)return out.dump.apply(this,messages);}
...
static assert(assertion,...messages){formatMessage(messages,(this===Dumper?undefined:this.#context),{color:'Crimson'});const new_ts=Date.now();const old_ts=this._last_assert||0;const options=this===Dumper?defaults:this.#options;if(options.assert.limit&&old_ts&&new_ts-old_ts<options.assert.limit)return!1;if(options.assert.time){messages.push(new_ts);const gap_ts=new_ts-old_ts;if(options.assert.hhmmss)messages.push((new Date(gap_ts).toISOString().substring(11,8)).replace(new RegExp('^(00:)+','gm'),''));else messages.push(gap_ts)}
...
const defaults={clear:!1,level:Dumper.WARN,bubbling:{listen:!0,trigger:!0,},trickle:1000,assert:{time:!1,hhmmss:!1,limit:0,}};if(!globalThis.Dumper){globalThis.Dumper=Dumper}else if(globalThis.Dumper!==Dumper){Dumper=globalThis.Dumper}
