...
((window, dumper) => {
    ...
    const showNotice = function (message, options = {}) {
        ...
        const remove = (targetNotice = notice) => {
            ...
            setTimeout(() => targetNotice.remove(), 200);
        };
    ...
    shortcut
        .add('shift + alt + r', () => {
            ...
            if (b === 0) showNotice('No blockers found.', { title: title, type: 'warning', icon: '⚠️', timeout: 5000 });
            else showNotice(`Possible blockers removed: <strong>${b}</strong>`, { title: title, type: 'success', icon: '❎', timeout: 5000 });
        }, 'Remove Blocker.')
